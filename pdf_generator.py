...
<<<<<<< HEAD
            ...
            original_projects = context.get("projects", [])
            ai_projects = ai.get("projects", [])
            if ai_projects:
                ai_by_name = {p.get("name", "").lower(): p for p in ai_projects}
                merged = []
                for proj in original_projects:
                    proj_copy = dict(proj) if isinstance(proj, dict) else proj
                    proj_name = proj_copy.get("name", "").lower()
                    ai_match = ai_by_name.get(proj_name, {})
                    if ai_match.get("enhanced_description"):
                        proj_copy["enhanced_description"] = ai_match["enhanced_description"]
                    merged.append(proj_copy)
                context["projects"] = merged
                # Keep ai_content.projects in sync so templates that reference it
                # also get the fully merged data (images, tech_stack, etc.)
                if isinstance(context.get("ai_content"), dict):
